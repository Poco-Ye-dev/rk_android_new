...
bool
dhd_conf_he_chip(dhd_pub_t *dhd)
{
	uint chip = dhd->conf->chip;

	if (chip == BCM43430_CHIP_ID || chip == BCM4345_CHIP_ID ||
			chip == BCM4356_CHIP_ID ||
...
bool
dhd_conf_eht_chip(dhd_pub_t *dhd)
{
	uint chip = dhd->conf->chip;
...
const sub_cmd_t *
dhd_conf_get_sub_cmd(char *cmd, char *sub_cmd)
{
	const sub_cmd_t *sub = NULL;

	if (!strcmp(cmd, "he"))
		sub = wl_find_sub_cmd(sub_cmd, he_cmd_list, ARRAY_SIZE(he_cmd_list));
	else if (!strcmp(cmd, "eht"))
		sub = wl_find_sub_cmd(sub_cmd, eht_cmd_list, ARRAY_SIZE(eht_cmd_list));
#ifdef WL_MLO
	else if (!strcmp(cmd, "mlo"))
		sub = wl_find_sub_cmd(sub_cmd, mlo_cmd_list, ARRAY_SIZE(mlo_cmd_list));
#endif /* WL_MLO */
...
dhd_conf_xtlv_cmd(dhd_pub_t *dhd, char *cmd, char *buf)
{
	...
	sub = dhd_conf_get_sub_cmd(cmd, sub_cmd_str);
	if (sub) {
		CONFIG_INFO("%s %s\n", cmd, buf);
...
const iovar_tpl_t iovar_tpl_list[] = {
	...
	{WLC_SET_VAR,	"eht",			dhd_conf_xtlv_cmd},
#ifdef WL_MLO
...
#if defined(WL_OWE) && !defined(WL_AP_OWE_BKPORT)
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 2, 0))
	CONFIG_ERROR("WL_AP_OWE_BKPORT not patched\n");
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(5, 2, 0) */
#endif /* WL_OWE && !WL_AP_OWE_BKPORT */
