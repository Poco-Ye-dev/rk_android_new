...
#ifdef CUSTOMER_HW_ROCKCHIP
	adapter->irq_num = rockchip_wifi_get_oob_irq();
#endif /* CUSTOMER_HW_ROCKCHIP */
...
#ifdef CUSTOMER_HW_ROCKCHIP
	irq_flags = IORESOURCE_IRQ | IORESOURCE_IRQ_SHAREABLE;
	flags = rockchip_wifi_get_oob_irq_flag();
	if (flags == 1)
		irq_flags |= IORESOURCE_IRQ_HIGHLEVEL;
	else if (flags == 0)
		irq_flags |= IORESOURCE_IRQ_LOWLEVEL;
	else
