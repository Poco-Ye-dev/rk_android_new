...
#define TX_TOFW(tx, pkthash)	((tx) ? ((pkthash) ? " (TO-FW) " : " (TO-DHD)") : "")
#endif /*
...
#define EAP_PRINT(x, args...) \
	do { \
		if (dump_msg_level & DUMP_EAPOL_VAL) { \
			if (tx) { \
				DHD_PKTDUMP(("[%s] 802_1X " x " [TX] : (%s) %s (%s)%s"TXFATE_FMT"\n", \
					ifname, ## args, \
					tx?seabuf:deabuf, tx?"->":"<-", tx?deabuf:seabuf, \
					TX_TOFW(tx, pkthash), TX_PKTHASH(pkthash), TX_FATE(pktfate))); \
			} else { \
...
#define EAP_PRINT_REPLAY(x, args...) \
	do { \
		if (dump_msg_level & DUMP_EAPOL_VAL) { \
			if (tx) { \
				DHD_PKTDUMP(("[%s] 802_1X " x " [TX] : (%s) %s (%s)%s"DBGREPLAY TXFATE_FMT"\n", \
					ifname, ## args, \
					tx?seabuf:deabuf, tx?"->":"<-", tx?deabuf:seabuf, \
					TX_TOFW(tx, pkthash), REPLAY_FMT(eap_key), TX_PKTHASH(pkthash), TX_FATE(pktfate))); \
			} else { \
...
#define EAP_PRINT_OTHER_4WAY(x, args...) \
	do { \
		if (dump_msg_level & DUMP_EAPOL_VAL) { \
			if (tx) { \
				DHD_PKTDUMP(("[%s] 802_1X " x " [TX] : (%s) %s (%s) " \
					"ver %d type %d keytype %d keyinfo 0x%02X%s"TXFATE_FMT"\n", \
					ifname, ## args, \
					tx?seabuf:deabuf, tx?"->":"<-", tx?deabuf:seabuf, \
	...
	bool tx, uint32 *pkthash, uint16 *pktfate)
{
...
#ifndef DHD_PKTDUMP_TOFW
	if (tx && pkthash && !(dump_msg_level & DUMP_TOFW_VAL))
		return;
	...
	dhd_trx_pkt_dump(dhdp, ifidx, pktdata, pktlen, pkthash, tx);
}
...
#define DHCP_PRINT(str) \
	do { \
		if (tx) { \
			DHD_PKTDUMP(("[%s] " str " %8s[%8s] [TX] : %s(%s) %s %s(%s)%s"TXFATE_FMT"\n", \
...
#define ICMP_PING_PRINT(str) \
	do { \
		if (tx) { \
			DHD_PKTDUMP_MEM(("[%s] "str " [TX] : %s(%s) %s %s(%s) SEQNUM=%d%s" \
...
#define ICMP_PRINT(str) \
	do { \
		if (tx) { \
			DHD_PKTDUMP_MEM(("[%s] "str " [TX] : %s(%s) %s %s(%s) TYPE=%d, CODE=%d%s" \
				TXFATE_FMT "\n", ifname, tx?sabuf:dabuf, tx?seabuf:deabuf, \
	...
	do { \
		if (tx) { \
			if (dump_enabled && pktfate && !TX_FATE_ACKED(pktfate)) { \
				DHD_PKTDUMP(("[%s] "str " [TX] : %s(%s) %s %s(%s)%s"TXFATE_FMT"\n", \
					ifname, tx?sabuf:dabuf, tx?seabuf:deabuf, \
					tx?"->":"<-", tx?dabuf:sabuf, tx?deabuf:seabuf, \
					TX_TOFW(tx, pkthash), TX_PKTHASH(pkthash), TX_FATE(pktfate))); \
			} else { \
				DHD_PKTDUMP_MEM(("[%s] "str " [TX] : %s(%s) %s %s(%s)%s"TXFATE_FMT"\n", \
					ifname, tx?sabuf:dabuf, tx?seabuf:deabuf, \
					tx?"->":"<-", tx?dabuf:sabuf, tx?deabuf:seabuf, \
					TX_TOFW(tx, pkthash), TX_PKTHASH(pkthash), TX_FATE(pktfate))); \
			} \
...
#define ARP_PRINT_OTHER(str) \
	do { \
		if (tx) { \
			if (dump_enabled && pktfate && !TX_FATE_ACKED(pktfate)) { \
				DHD_PKTDUMP(("[%s] "str " [TX] : %s(%s) %s %s(%s) op_code=%d%s" \
					TXFATE_FMT "\n", ifname, tx?sabuf:dabuf, tx?seabuf:deabuf, \
					tx?"->":"<-", tx?dabuf:sabuf, tx?deabuf:seabuf, opcode, \
					TX_TOFW(tx, pkthash), TX_PKTHASH(pkthash), TX_FATE(pktfate))); \
			} else { \
				DHD_PKTDUMP_MEM(("[%s] "str " [TX] : %s(%s) %s %s(%s) op_code=%d%s" \
				TXFATE_FMT "\n", ifname, tx?sabuf:dabuf, tx?seabuf:deabuf, \
					tx?"->":"<-", tx?dabuf:sabuf, tx?deabuf:seabuf, opcode, \
				TX_TOFW(tx, pkthash), TX_PKTHASH(pkthash), TX_FATE(pktfate))); \
			} \
...
#define DNS_REQ_PRINT(str) \
	do { \
		if (tx) { \
			if (dump_enabled && pktfate && !TX_FATE_ACKED(pktfate)) { \
				DHD_PKTDUMP(("[%s] " str " [TX] : %s(%s) %s %s(%s) ID:0x%04X OPCODE:%s%s" \
					TXFATE_FMT "\n", ifname, tx?sabuf:dabuf, tx?seabuf:deabuf, \
					tx?"->":"<-", tx?dabuf:sabuf, tx?deabuf:seabuf, \
					id, DNSOPCODE(opcode), TX_TOFW(tx, pkthash), TX_PKTHASH(pkthash), TX_FATE(pktfate))); \
			} else { \
				DHD_PKTDUMP_MEM(("[%s] " str " [TX] : %s(%s) %s %s(%s) ID:0x%04X OPCODE:%s%s" \
					TXFATE_FMT "\n", ifname, tx?sabuf:dabuf, tx?seabuf:deabuf, \
					tx?"->":"<-", tx?dabuf:sabuf, tx?deabuf:seabuf, \
					id, DNSOPCODE(opcode), TX_TOFW(tx, pkthash), TX_PKTHASH(pkthash), TX_FATE(pktfate))); \
			} \
...
#define DNS_RESP_PRINT(str) \
	do { \
		if (tx) { \
			if (dump_enabled && pktfate && !TX_FATE_ACKED(pktfate)) { \
				DHD_PKTDUMP(("[%s] " str " [TX] : %s(%s) %s %s(%s) ID:0x%04X OPCODE:%s RCODE:%d%s" \
					TXFATE_FMT "\n", ifname, tx?sabuf:dabuf, tx?seabuf:deabuf, \
					tx?"->":"<-", tx?dabuf:sabuf, tx?deabuf:seabuf, id, DNSOPCODE(opcode), \
					GET_DNS_RCODE(flags), TX_TOFW(tx, pkthash), TX_PKTHASH(pkthash), TX_FATE(pktfate))); \
			} else { \
				DHD_PKTDUMP_MEM(("[%s] " str " [TX] : %s(%s) %s %s(%s) ID:0x%04X OPCODE:%s RCODE:%d%s" \
					TXFATE_FMT "\n", ifname, tx?sabuf:dabuf, tx?seabuf:deabuf, \
					tx?"->":"<-", tx?dabuf:sabuf, tx?deabuf:seabuf, id, DNSOPCODE(opcode), \
					GET_DNS_RCODE(flags), TX_TOFW(tx, pkthash), TX_PKTHASH(pkthash), TX_FATE(pktfate))); \
			} \
...
#ifdef DHD_TRX_DUMP
void
dhd_trx_pkt_dump(dhd_pub_t *dhdp, int ifidx, uint8 *pktdata, uint32 pktlen,
	uint32 *pkthash, bool tx)
{
	struct ether_header *eh;
	uint16 protocol;
	char *pkttype;
	...
	pkttype = tx ? "[TX] UNKNOWN" : "[RX] UNKNOWN";
	switch (protocol) {
	case ETHER_TYPE_IP:
		if (dhd_check_dhcp(pktdata))
			pkttype = tx ? "[TX] DHCP" : "[RX] DHCP";
		else if (dhd_check_icmp(pktdata))
			pkttype = tx ? "[TX] ICMP" : "[RX] ICMP";
		else if (dhd_check_dns(pktdata))
			pkttype = tx ? "[TX] DNS" : "[RX] DNS";
		else
			pkttype = tx ? "[TX] IP" : "[RX] IP";
		break;
	case ETHER_TYPE_ARP:
		pkttype = tx ? "[TX] ARP" : "[RX] ARP";
		break;
	case ETHER_TYPE_BRCM:
		pkttype = tx ? "[TX] BRCM" : "[RX] BRCM";
		break;
	case ETHER_TYPE_802_1X:
		pkttype = tx ? "[TX] 802.1X" : "[RX] 802.1X";
		break;
	case ETHER_TYPE_WAI:
		pkttype = tx ? "[TX] WAPI" : "[RX] WAPI";
		break;
	default:
		break;
	}
	...
	if (protocol != ETHER_TYPE_BRCM) {
		if (pktdata[0] == 0xFF) {
			DHD_PKTDUMP(("[%s] BROADCAST DUMP -  %s%s\n",
				dhd_ifname(dhdp, ifidx), pkttype, TX_TOFW(tx, pkthash)));
		} else if (pktdata[0] & 1) {
			DHD_PKTDUMP(("[%s] MULTICAST DUMP " MACDBG " - %s%s\n",
				dhd_ifname(dhdp, ifidx), MAC2STRDBG(pktdata),
				pkttype, TX_TOFW(tx, pkthash)));
		} else {
			DHD_PKTDUMP(("[%s] DUMP - %s%s\n",
				dhd_ifname(dhdp, ifidx), pkttype, TX_TOFW(tx, pkthash)));
		}
#ifdef DHD_TRX_FULL_DUMP
		prhex(pkttype, pktdata, pktlen);
#endif /* DHD_TRX_FULL_DUMP */
	}
	else {
		DHD_PKTDUMP(("[%s] DUMP - %s%s\n",
			dhd_ifname(dhdp, ifidx), pkttype, TX_TOFW(tx, pkthash)));
