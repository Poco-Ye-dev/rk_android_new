...
int
wl_ext_get_xtlv_uint(struct net_device *dev, char *cmd, char *sub_cmd, int *val)
{
	char iovar_buf[WLC_IOCTL_SMLEN];
	const sub_cmd_t *sub;
	bcm_xtlv_t read_he_xtlv;
	int ret = BCME_NOTFOUND, len;

	sub = dhd_conf_get_sub_cmd(cmd, sub_cmd);
	if (sub) {
		read_he_xtlv.id = sub->id;
		read_he_xtlv.len = 0;
		ret = wldev_iovar_getbuf(dev, cmd, &read_he_xtlv, sizeof(read_he_xtlv),
			iovar_buf, WLC_IOCTL_SMLEN, NULL);
		if (!ret) {
			*val = 0;
			len = wl_iovt2len(sub->type);
			memcpy(val, iovar_buf, len);
		}
	}
