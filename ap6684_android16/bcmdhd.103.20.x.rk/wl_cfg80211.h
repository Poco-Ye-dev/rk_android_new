...
#endif /* (LINUX_VERSION_CODE < KERNEL_VERSION(5, 3, 0)) && !NL80211_WPA_VERSION_3*/
...
do {	\
	if (wl_dbg_level & WL_DBG_ERR) {	\
		...
		(__cur_ts - __err_ts > WL_PRINT_RATE_LIMIT_PERIOD))) { \
			__err_ts = __cur_ts; \
			WL_ERR(("[Repeats %u times last time] " x, __err_cnt, ## args)); \
			__err_cnt = 0; \
		} else { \
			++__err_cnt; \
		} \
...
#define WL_ERR_RLMT(x) WL_ERR_RLMT_MSG x
...
#define CFG80211_CAC_EVENT(dev, chandef, event, KMALLOC_FLAG, link_id) \
	cfg80211_cac_event(dev, chandef, event, KMALLOC_FLAG, link_id);
#else
#define CFG80211_CAC_EVENT(dev, chandef, event, KMALLOC_FLAG, link_id) \
	cfg80211_cac_event(dev, chandef, event, KMALLOC_FLAG);
