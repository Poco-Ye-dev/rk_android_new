...
int set_controls(struct mixer *mixer, const struct config_control *ctls, const unsigned ctls_count)
{
    ...
    for (i = 0; i < ctls_count; i++) {
        ...
        if (!ctl) {
            ALOGE_IF(route_table != &default_config_table, "set_controls() Can not get ctl : %s xxx", ctls[i].ctl_name);
            ALOGV_IF(route_table == &default_config_table, "set_controls() Can not get ctl : %s xxx", ctls[i].ctl_name);
        ...
        if (ctl->info->type != SNDRV_CTL_ELEM_TYPE_BOOLEAN &&
            ...
            ALOGE("set_controls() ctl %s is not a type of INT or ENUMERATED xxx", ctls[i].ctl_name);
            continue;//return -EINVAL;
        }

        if (ctls[i].str_val) {
            if (ctl->info->type != SNDRV_CTL_ELEM_TYPE_ENUMERATED) {
                ALOGE("set_controls() ctl %s is not a type of ENUMERATED xxx", ctls[i].ctl_name);
                continue;//return -EINVAL;
            }
            if (mixer_ctl_select(ctl, ctls[i].str_val) != 0) {
                ALOGE("set_controls() Can not set ctl %s to %s xxx", ctls[i].ctl_name, ctls[i].str_val);
                continue;//return -EINVAL;
            }
            ...
            if (mixer_ctl_set_int_double(ctl, ctls[i].int_val[0], ctls[i].int_val[1]) != 0) {
                ALOGE("set_controls() can not set ctl %s to %d xxx", ctls[i].ctl_name, ctls[i].int_val[0]);
                continue;//return -EINVAL;
            }
