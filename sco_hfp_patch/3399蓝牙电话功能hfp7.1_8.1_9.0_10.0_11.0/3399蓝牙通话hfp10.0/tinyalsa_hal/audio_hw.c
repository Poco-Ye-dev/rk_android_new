...
//#define ALSA_IN_BT_DEBUG
//#define ALSA_DEBUG
#ifdef ALSA_IN_DEBUG
FILE *in_debug;
#endif

#ifdef ALSA_IN_BT_DEBUG
FILE *in_bt_debug = NULL;
#endif
...
static int start_output_stream(struct stream_out *out)
{
	...
	if (out->device & AUDIO_DEVICE_OUT_ALL_SCO) {
		...
		if(card != (int)SND_OUT_SOUND_CARD_UNKNOWN) {
            ...
            ALOGD("out config rate %d = %d", out->config.rate, pcm_config_ap_sco.rate);
                           ...
                           struct resampler_buffer* buffer)
{
    ...
    if (in->frames_in == 0) {
        ...
        read_size = pcm_frames_to_bytes(in->pcm, in->config->period_size);
        ...
        if (in->device & AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET) {
            if (in_bt_debug) {
                fwrite(in->buffer, read_size, 1, in_bt_debug);
            } else {
                ALOGE("in_bt_debug is null");
            }
        }
#endif
...
#ifdef ALSA_IN_BT_DEBUG
    in_bt_debug = fopen("/data/debug_bt.pcm","wb");//please touch /data/debug.pcm first
#endif
